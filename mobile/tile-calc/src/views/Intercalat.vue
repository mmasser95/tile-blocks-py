...
const tileWidth = ref(25)
const tileHeight = ref(25)
...
const calcTiles = () => {
    ...
    // Crear baldosas con desplazamiento para las filas impares
    for (let i = 0; i < tiles_x; i++) {
        for (let j = 0; j < tiles_y; j++) {
            let x = i * tileWidth.value
            let y = j * tileHeight.value

            // Desplazamiento en las filas impares
            if (j % 2 !== 0) {
                x += tileWidth.value / 2; // Desplazamos la fila impar
            }
            ...
            if(j%2!==0&&i==0){
                result.push({x:x-tileWidth.value/2,y,w:w/2,h,complete:false})
            }
